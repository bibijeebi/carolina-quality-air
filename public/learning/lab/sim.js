...
(function () {
  ...
  // Unanswered items are stored as misses with no answer (a == null).
  const skipped = s => (s.miss || []).filter(m => m[1] == null).length;
  const histLine = s => { const sk = skipped(s); return `${esc(s.label || '')} ${s.total}q${sk ? ` · ${sk} skipped` : ''}`; };
  ...
  function setup(D) {
    ...
    const act = S.sim;
    // The ready check decides which length is next: short sims until two in a row at 85%, then the full length.
    const rd = V.readiness(D, S), next = rd.stepOne && !rd.stepTwo ? rd.full : !rd.stepOne ? rd.quick : null;
    V.main.innerHTML = hd
      ...
      + `<div class="sim-grid">${lens.map(([n, label]) => { const m = Math.round((n * secsPerQ(D)) / 60); return `<section class="panel sim-card${Number(n) === next ? ' next' : ''}"><p class="eyebrow">${esc(label)}${Number(n) === next ? ' · <span class="next-tag">Next for your ready check</span>' : ''}</p><h2>${n} questions</h2><p class="muted">${m} minutes${Number(n) === Number(e.questions) && V.ePub(D) ? ', the real exam length' : ''}.</p><button type="button" class="btn${act ? ' secondary' : ''}" data-len="${n}" data-label="${esc(label)}"${act ? ' disabled' : ''}>Start<span class="hide-sm"> ${n}-question sim</span></button></section>`; }).join('')}</div>`
      ...
      + `<section class="panel"><h2>Sim history</h2>${S.sims.length ? `<div class="history">${S.sims.slice().reverse().map((s, i) => `<a class="history-row" href="${link(id, 'sim', 'report', S.sims.length - 1 - i)}"><span>${fmtDate(s.ts)} · ${histLine(s)}</span><b class="${s.pct >= (Number(e.cut) || 0) ? 'ok' : 'bad'}">${s.pct}%</b></a>`).join('')}</div>` : '<p class="muted space">No sims yet.</p>'}</section></div>`;
  ...
  function run(D) {
    ...
    V.main.innerHTML = `<div class="quiz-wrap"><div class="quiz-top"><a class="btn text small" href="${link(id, 'sim')}">Save and leave</a><span class="qlabel">${esc(sim.label)} · ${answered}/${sim.items.length} answered</span><strong id="sim-clock" role="timer" aria-label="Time left">${clock((sim.end - Date.now()) / 1000)}</strong></div><div class="progress" aria-hidden="true"><i style="width:${pct(answered, sim.items.length)}%"></i></div>`
      + `<section class="panel question-panel"><div class="row-between"><p class="eyebrow">Question ${sim.cur + 1} of ${sim.items.length}<span class="sim-answered"> · ${answered} answered</span></p><button type="button" class="toggle flag" id="flag" aria-pressed="${!!it.f}"><span class="knob" aria-hidden="true"></span>Flag</button></div><h2 class="quiz-title">${esc(q.stem)}</h2>${V.choicesHTML(q, { order: it.o, sel: it.a }, 'sim')}`
      + `<div class="quiz-bottom q-bar"><button type="button" class="btn secondary" id="prev"${sim.cur ? '' : ' disabled'}>Previous</button><button type="button" class="btn" id="nextq">${sim.cur + 1 < sim.items.length ? 'Next' : 'Review and submit'}</button></div></section>`
  ...
  function report(D, idx) {
    ...
    const missHTML = (s.miss || []).map(([qid, a], k) => {
      ...
      return `<details class="review-item"${k < 3 ? ' open' : ''}><summary><span class="status">${a == null ? 'Skipped' : 'Missed'} · ${esc(q.domain)}${q.reserved ? ' · sim only' : ''}</span><span class="rv-stem">${esc(q.stem)}</span></summary>${a != null ? `<p><b>Your answer:</b> ${esc(q.options[a])}</p>` : ''}<p><b>Correct:</b> ${esc(q.options[0])}</p><p class="muted">${esc(q.expl)}</p>${V.testsHTML(D, q)}</details>`;
    }).join('');
    const mins = Math.round((s.secs || 0) / 60);
    // Every domain under the line is listed. Ones with no practice questions show without a drill link.
    const weak = Object.entries(s.dom).map(([n, [r, t]]) => [n, pct(r, t)]).filter(([, p]) => p < cut).sort((a, b) => a[1] - b[1]);
    const weakHTML = weak.map(([n, p]) => D.pool.some(q => q.domain === n) ? `<a class="btn secondary small" href="${link(id, 'drill', 'dom', n)}">${esc(n)} (${p}%)</a>` : `<span class="chip weak-chip">${esc(n)} (${p}%) · sim only</span>`).join('');
    const sk = skipped(s), answered = s.total - sk, wrong = Math.max(0, answered - s.right);
    const tally = `${s.right} right, ${wrong} wrong, ${sk} skipped`;
    const basis = sk
      ? `You answered ${answered} of ${s.total}. The score counts all ${s.total}, so skipped questions count against you, as on the exam.${answered ? ` On the ones you answered you got ${pct(s.right, answered)}%.` : ''}`
      : `You answered all ${s.total}.`;
    V.main.innerHTML = head(`${esc(tr.code)} / Sim report`, `${s.pct}%. ${where} the ${cut}% line.`, `${esc(s.label || 'Sim')}, ${s.total} questions, ${fmtDate(s.ts)}${mins ? `, ${mins} min used` : ''}${s.how === 'time' ? ', submitted when time ran out' : ''}.`)
      + `<div class="grid"><section class="panel"><div class="result-top"><div class="result-score ${above ? 'ok' : 'bad'}">${s.pct}%</div><div><h2>${tally}</h2><p class="small space-sm">${basis}</p><p class="muted small space-sm">${esc((tr.exam || {}).cutNote || '')}</p></div></div><h3 class="space">By domain</h3><p class="small muted">The line on each bar marks the ${cut}% cut.</p><div class="domain-list">${rows}</div></section>`
      + `<section class="panel"><h2>Next move</h2>${s.miss && s.miss.length ? `<p class="space-sm">${[wrong ? plural(wrong, 'wrong answer') : '', sk ? `${sk} skipped` : ''].filter(Boolean).join(' and ')}.${drillable.size ? ` ${plural(drillable.size, 'practice question')} ${drillable.size === 1 ? 'covers' : 'cover'} them.` : ' They are sim-only questions with no practice questions on the same facts yet.'}</p>` : '<p class="space-sm">No misses. Try a longer sim.</p>'}<div class="actions space">${drillable.size ? `<a class="btn" href="${link(id, 'drill', 'sim', i)}">Drill these misses</a>` : ''}<a class="btn secondary" href="${link(id, 'sim')}">Another sim</a><a class="btn text" href="${link(id, 'progress')}">Progress</a></div>${weak.length ? `<h3 class="space-lg">Below the line</h3><p class="small muted">${plural(weak.length, 'domain')} scored under ${cut}%. Drill ${weak.length === 1 ? 'it' : 'them'}, weakest first.</p><div class="actions space-sm">${weakHTML}</div>` : ''}</section></div>`
      + (missHTML ? `<section class="panel space-lg"><div class="row-between"><h2>${sk && wrong ? 'Missed and skipped' : sk ? 'Skipped items' : 'Missed items'}</h2><button type="button" class="btn text small" id="rv-all">Open all</button></div>${missHTML}</section>` : '')
      + `<section class="panel space-lg"><h2>Sim history</h2><div class="history">${S.sims.slice().reverse().map((x, k) => { const j = S.sims.length - 1 - k; return `<a class="history-row${j === i ? ' current' : ''}" href="${link(id, 'sim', 'report', j)}"><span>${fmtDate(x.ts)} · ${histLine(x)}</span><b class="${x.pct >= cut ? 'ok' : 'bad'}">${x.pct}%</b></a>`; }).join('')}</div></section>`;
